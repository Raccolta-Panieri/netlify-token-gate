...
exports.handler = async function(event) {
  try {
    ...
    // ---------- supporto set_default: se richiesto, salviamo il token plain in Upstash ----------
    try {
      ...
      if (shouldSetDefault) {
        // scriviamo il token plain in key 'default_public_token' con stessa EX=ttl
        const defaultSetRes = await fetch(`${UPSTASH_REST_URL}/set/default_public_token?EX=${ttl}`, {
          method: 'POST',
          headers: {
            "Authorization": `Bearer ${UPSTASH_REST_TOKEN}`,
            "Content-Type": "text/plain"
        ...
        if (!defaultSetRes.ok) {
          // logga ma non fallire la richiesta principale
          const txt = await defaultSetRes.text().catch(()=>'<no body>');
          console.warn('generate-token: set default_public_token failed', defaultSetRes.status, txt);
        }
      }
    } catch (e) {
      console.warn('generate-token: exception while setting default_public_token', e);
